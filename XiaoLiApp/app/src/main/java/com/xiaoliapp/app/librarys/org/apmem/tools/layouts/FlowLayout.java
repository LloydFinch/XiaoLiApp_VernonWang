package com.xiaoliapp.app.librarys.org.apmem.tools.layouts;
...
public class FlowLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (newLine) {
				...
				if (this.config.getOrientation() == VERTICAL && this.config.getLayoutDirection() ==
			...
			if (this.config.getOrientation() == HORIZONTAL && this.config.getLayoutDirection() ==
					LAYOUT_DIRECTION_RTL) {
		...
		this.setMeasuredDimension(resolveSize(totalControlWidth, widthMeasureSpec), resolveSize(totalControlHeight,
				heightMeasureSpec));
	}
	...
	private void applyGravityToLine(LineDefinition line) {
		...
		int excessLength = line.getLineLength() - (lastChildLayoutParams.getLength() + lastChildLayoutParams
